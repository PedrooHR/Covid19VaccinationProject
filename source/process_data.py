...
# Application Methods
# ###########################################
def downloadData(online_data):
    print("\n----------------------------------------------")
    for data in online_data:
        print(f"Downloading {data['url']}")
        request.urlretrieve(data['url'], data['output'])
...
if __name__ == '__main__':
    ...
    # Raw data urls
    vaccination_url = "https://github.com/wcota/covid19br-vac/raw/main/"
    covid_url = "https://github.com/wcota/covid19br/raw/master/"

    # Series that describe if we need to download any data
    online_data = []
    ...
    # Before processing data, lets see if we need to download any data
    for file in vaccination_files:
        if not os.path.exists(file):
            online_data.append(
                {'url': vaccination_url + file.split('/')[-1], 'output': file})

    for file in covid_files:
        if not os.path.exists(file):
            online_data.append(
                {'url': covid_url + file.split('/')[-1], 'output': file})

    if len(online_data) > 0:
        downloadData(online_data)
